...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := hcashutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(hcashutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := hcashutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := hcashutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
...
func TestFilterInsertP2PubKeyOnly(t *testing.T) {
	...
	block, err := hcashutil.NewBlockFromBytes(blockBytes)
	if err != nil {
