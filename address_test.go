...
func TestAddresses(t *testing.T) {
	tests := []struct {
		...
		encoded string
		valid   bool
		result  hcashutil.Address
		f       func() (hcashutil.Address, error)
		net     *chaincfg.Params
	}{
		// Positive P2PKH tests.
		{
			...
			valid:   true,
			result: hcashutil.TstAddressPubKeyHash(
				[ripemd160.Size]byte{
			...
			f: func() (hcashutil.Address, error) {
				pkHash := []byte{
					0x27, 0x89, 0xd5, 0x8c, 0xfa, 0x09, 0x57, 0xd2, 0x06, 0xf0,
					0x25, 0xc2, 0xaf, 0x05, 0x6f, 0xc8, 0xa7, 0x7c, 0xeb, 0xb0}
				return hcashutil.NewAddressPubKeyHash(pkHash,
		...
		// Positive P2SH tests.
		{
			...
			valid:   true,
			result: hcashutil.TstAddressScriptHash(
				[ripemd160.Size]byte{
			...
			f: func() (hcashutil.Address, error) {
				...
				return hcashutil.NewAddressScriptHash(txscript, &chaincfg.MainNetParams)
			},
			net: &chaincfg.MainNetParams,
		},
		{
			...
			f: func() (hcashutil.Address, error) {
				hash := []byte{
					0xc7, 0xda, 0x50, 0x95, 0x68, 0x34, 0x36, 0xf4, 0x43, 0x5f,
					0xc4, 0xe7, 0x16, 0x3d, 0xca, 0xfd, 0xa1, 0xa2, 0xd0, 0x07}
				return hcashutil.NewAddressScriptHashFromHash(hash, &chaincfg.MainNetParams)
			},
			net: &chaincfg.MainNetParams,
		},
		{
			...
			f: func() (hcashutil.Address, error) {
				hash := []byte{
					0x36, 0xc1, 0xca, 0x10, 0xa8, 0xa6, 0xa4, 0xb5, 0xd4, 0x20,
					0x4a, 0xc9, 0x70, 0x85, 0x39, 0x79, 0x90, 0x3a, 0xa2, 0x84}
				return hcashutil.NewAddressScriptHashFromHash(hash, &chaincfg.TestNet2Params)
			},
			net: &chaincfg.TestNet2Params,
		},
		...
		// Positive P2PK tests.
		{
			...
			valid:   true,
			result: hcashutil.TstAddressPubKey(
				[]byte{
					...
					0x0f, 0xe2, 0xed},
				hcashutil.PKFCompressed, chaincfg.MainNetParams.PubKeyHashAddrID),
			f: func() (hcashutil.Address, error) {
				serializedPubKey := []byte{
					...
					0x52, 0xe7, 0xfe, 0xf0, 0xea, 0x6d, 0xa1, 0x43, 0xa0, 0x2b,
					0x0f, 0xe2, 0xed}
				return hcashutil.NewAddressSecpPubKey(serializedPubKey, &chaincfg.MainNetParams)
			},
			net: &chaincfg.MainNetParams,
		},
		{
		...
		/* XXX currently commented out due to issues with the tests that result in these errors:
		    ...
		    We are currently only handle compressed keys in hcashd, but the protocol does support
		    hybrid and uncompressed so users may try to implement at some point
		    {
				...
				result: hcashutil.TstAddressPubKey(
					...
					hcashutil.PKFUncompressed, chaincfg.MainNetParams.PubKeyHashAddrID),
				f: func() (hcashutil.Address, error) {
					serializedPubKey := []byte{
						0x04, 0x64, 0xc4, 0x46, 0x53, 0xd6, 0x56, 0x7e, 0xff, 0x57,
						0x53, 0xc5, 0xd2, 0x4a, 0x68, 0x2d, 0xdc, 0x2b, 0x2c, 0xad,
			...
			},
			{
				...
				result: hcashutil.TstAddressPubKey(
					[]byte{
						...
						0xa7, 0x94, 0x14, 0x3d, 0x7c, 0xfd, 0x3e, 0x80, 0x80, 0x8a,
						0x1f, 0xa3, 0x20, 0x39, 0x04},
					hcashutil.PKFHybrid, chaincfg.MainNetParams.PubKeyHashAddrID),
				f: func() (hcashutil.Address, error) {
					serializedPubKey := []byte{
						0x06, 0x64, 0xc4, 0x46, 0x53, 0xd6, 0x56, 0x7e, 0xff, 0x57,
						0x53, 0xc5, 0xd2, 0x4a, 0x68, 0x2d, 0xdc, 0x2b, 0x2c, 0xad,
		...
		*/
		{
			...
			result: hcashutil.TstAddressPubKey(
				[]byte{
					...
					0x4b, 0xe0, 0x6e},
				hcashutil.PKFCompressed, chaincfg.TestNet2Params.PubKeyHashAddrID),
			f: func() (hcashutil.Address, error) {
				serializedPubKey := []byte{
					0x02, 0x6a, 0x40, 0xc4, 0x03, 0xe7, 0x46, 0x70, 0xc4, 0xde,
					0x76, 0x56, 0xa0, 0x9c, 0xaa, 0x23, 0x53, 0xd4, 0xb3, 0x83,
					0xa9, 0xce, 0x66, 0xee, 0xf5, 0x1e, 0x12, 0x20, 0xea, 0xcf,
					0x4b, 0xe0, 0x6e}
				return hcashutil.NewAddressSecpPubKey(serializedPubKey, &chaincfg.TestNet2Params)
			},
			net: &chaincfg.TestNet2Params,
		},
		{
		...
		/*  XXX These are commented out for the same reasons above.
		{
			...
			result: hcashutil.TstAddressPubKey(
				...
				hcashutil.PKFUncompressed, chaincfg.TestNet2Params.PubKeyHashAddrID),
			f: func() (hcashutil.Address, error) {
				serializedPubKey := []byte{
					0x04, 0x6a, 0x40, 0xc4, 0x03, 0xe7, 0x46, 0x70, 0xc4, 0xde,
					0x76, 0x56, 0xa0, 0x9c, 0xaa, 0x23, 0x53, 0xd4, 0xb3, 0x83,
		...
		},
		{
			...
			result: hcashutil.TstAddressPubKey(
				[]byte{
					...
					0xd5, 0xa1, 0xfd, 0x2d, 0x77, 0x86, 0x7f, 0xca, 0x64, 0x21,
					0x7a, 0xce, 0xcf, 0x22, 0x44},
				hcashutil.PKFHybrid, chaincfg.TestNet2Params.PubKeyHashAddrID),
			f: func() (hcashutil.Address, error) {
				serializedPubKey := []byte{
					0x06, 0x6a, 0x40, 0xc4, 0x03, 0xe7, 0x46, 0x70, 0xc4, 0xde,
					0x76, 0x56, 0xa0, 0x9c, 0xaa, 0x23, 0x53, 0xd4, 0xb3, 0x83,
	...
	for _, test := range tests {
		...
		decoded, err := hcashutil.DecodeAddress(test.addr)
		...
		if err == nil {
			...
			case *hcashutil.AddressPubKeyHash:
				saddr = hcashutil.TstAddressSAddr(encoded)
