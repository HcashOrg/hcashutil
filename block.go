...
// Tx returns a wrapped transaction (hcashutil.Tx) for the transaction at the
...
// STx returns a wrapped transaction (hcashutil.Tx) for the stake transaction at
...
// Transactions returns a slice of wrapped transactions (hcashutil.Tx) for all
...
// instead provides easy access to wrapped versions (hcashutil.Tx) of them.
...
// STransactions returns a slice of wrapped stake transactions (hcashutil.Tx) for all
// stake transactions in the Block.  This is nearly equivalent to accessing the raw
// transactions (hcashwire.MsgTx) in the underlying wire.MsgBlock, however it
