...
func (cs *CoinSet) TotalValue() (value hcashutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue hcashutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue hcashutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount hcashutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue hcashutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue hcashutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue hcashutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue hcashutil.Amount, coins []Coin) (Coins, error) {
...
func (c *SimpleCoin) Value() hcashutil.Amount {
	return hcashutil.Amount(c.txOut().Value)
}
