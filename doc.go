...
A Block defines a hypercash block that provides easier and more efficient
...
A Tx defines a hypercash transaction that provides more efficient manipulation of
...
The Address interface provides an abstraction for a Hypercash address.  While the
most common type is a pay-to-pubkey-hash, Hypercash already supports others and
...
To decode/encode an address:
	...
	addr, err := hcashutil.DecodeAddress(addrString, defaultNet)
	if err != nil {
		fmt.Println(err)
		return
	}
