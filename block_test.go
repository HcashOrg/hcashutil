...
func TestBlock(t *testing.T) {
	b := hcashutil.NewBlock(&Block100000)
...
func TestNewBlockFromBytes(t *testing.T) {
	...
	b, err := hcashutil.NewBlockFromBytes(block100000Bytes)
	if err != nil {
...
func TestNewBlockFromBlockAndBytes(t *testing.T) {
	...
	// Create a new block from the serialized bytes.
	b := hcashutil.NewBlockFromBlockAndBytes(&Block100000, block100000Bytes)
...
func TestBlockErrors(t *testing.T) {
	...
	testErr := hcashutil.OutOfRangeError(wantErr)
	...
	_, err = hcashutil.NewBlockFromBytes(shortBytes)
	if err != io.EOF {
	...
	_, err = b.TxHash(-1)
	if _, ok := err.(hcashutil.OutOfRangeError); !ok {
		t.Errorf("TxHash: wrong error - got: %v <%T>, "+
			"want: <%T>", err, err, hcashutil.OutOfRangeError(""))
	}
